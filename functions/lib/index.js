...
app.get('/article/:articleId', (req, res) => {
    ...
    try {
        fs.readFile('./index.html', 'utf8', (err, htmlString) => {
            ...
            let replacedHTML = htmlString.replace(/<title>(.*?)<\/title>/, `<title>${title}</title>`);
            replacedHTML = replacedHTML.replace(/<\/head>/, `
...
exports.articleDetail = functions.https.onRequest(app);
